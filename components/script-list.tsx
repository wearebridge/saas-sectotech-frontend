...
export function ScriptList({ serviceSubTypeId, refreshTrigger }: ScriptListProps) {
  ...
  return (
    <>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {scripts.map((script) => (
          <div key={script.id} onClick={() => setEditingScript(script)} className="cursor-pointer">
            <Card className="h-full hover:bg-muted/50 transition-colors">
              <CardHeader>
                ...
                <CardDescription>
                    {script.scriptItems ? `${script.scriptItems.length} itens` : "0 itens"}
                </CardDescription>
              </CardHeader>
            </Card>
          </div>
      ...
      {editingScript && (
        <ScriptForm 
            ...
            initialData={{
                ...
                scriptItems: editingScript.scriptItems?.map(item => ({
